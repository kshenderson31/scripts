...
# Name: bos004.sh
#
# Description : Script invoked my CRON that generates the YTD balance of sales
...
if [ -s ${RESULTS_FILE} ]; then
  ...
  for row in $(grep code ${RESULTS_FILE}); do
    ...
    BOS_RPT_PATH=${TMP_PATH}/YTDDistBos${DIST_NBR}.pdf

    log "persisting YTD BOS report for district ${DIST_NBR} to ${BOS_RPT_PATH}"
    # generate the bos report and write it to tmp
    wget -q -O ${BOS_RPT_PATH} ${BASE_URL}/BosReport/services/reportService/bos/ytd/district/${DIST_NBR}

    RTN=$?

    if [ ${RTN} -ne 0 ]; then
      logError "An error occurred generating YTD District BOS report for ${DIST_NBR}, RC=${RTN}"
  ...
  ZIPFILE_NAME=YTDDistBosReports_$(date +%m%d%Y_%H%M%S).zip
  ZIPFILE_PATH="${TMP_PATH}/${ZIPFILE_NAME}"
  log "Creating ${ZIPFILE_PATH}"
#  jar -cMf ${ZIPFILE_PATH} ${TMP_PATH}/YTDDistBos*.pdf
  /prod/jre_1.4.2/bin/jar -cMf ${ZIPFILE_PATH} ${TMP_PATH}/YTDDistBos*.pdf
  ...
  (echo "Attached are the generated year to date balance of sales reports for all districts"; /usr/bin/uuencode ${ZIPFILE_PATH} ${ZIPFILE_NAME}) | mailx -r no-reply@theparadiesshops.com -s "Year to Date Balance of Sales Reports for all Districts" ${EMAIL_RECIPIENTS}
  ...
  log "Cleaning up YTD District Bos pdf files"
  rm ${TMP_PATH}/YTDDistBos*.pdf
fi # end if
