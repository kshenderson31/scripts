...
# Name: bos002.sh
#
# Description : Script invoked my CRON that generates the year to date balance 
#               of sales (bos) report for each retail center setup in GERS. It 
#               then saves these files in compressed form so they can be 
#               uploaded to the enterprise portal (accesstps or other)
#
...
"
  ...
  for row in $(cat ${RESULTS_FILE}); do
    RC_NBR=${row/ */}
    BOS_RPT_PATH=${TMP_PATH}/YTDBos${RC_NBR}.pdf

    log "persisting BOS report for ${RC_NBR} to ${BOS_RPT_PATH}"
    # generate the YTD bos report and write it to tmp
    wget -q -O ${BOS_RPT_PATH} http://10.0.0.3/BosReport/services/reportService/bos/ytd/${RC_NBR}

    RTN=$?

    if [ ${RTN} -ne 0 ]; then
      logError "An error occurred generating YTD BOS report for ${RC_NBR}, RC=${RTN}"
  ...
  ZIPFILE_NAME=YTDBosReports_$(date +%m%d%Y_%H%M%S).zip
  ZIPFILE_PATH="${TMP_PATH}/${ZIPFILE_NAME}"
  log "Creating ${ZIPFILE_PATH}"
#  jar -cMf ${ZIPFILE_PATH} ${TMP_PATH}/YTDBos*.pdf
  /prod/jre_1.4.2/bin/jar -cMf ${ZIPFILE_PATH} ${TMP_PATH}/YTDBos*.pdf
  ...
  (echo "Attached are the generated year to date balance of sales reports for all retail centers"; /usr/bin/uuencode ${ZIPFILE_PATH} ${ZIPFILE_NAME}) | mailx -r no-reply@theparadiesshops.com -s "Year to Date Balance of Sales Reports for all Retail Centers" ${EMAIL_RECIPIENTS}
  ...
  log "Cleaning up YTD Bos pdf files"
  rm ${TMP_PATH}/YTDBos*.pdf
fi # end if
