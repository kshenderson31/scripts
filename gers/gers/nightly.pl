...
where a.store_cd = ?
  ...
  and a.term_num = b.term_num
  and a.TRN_DT between (select TO_CHAR(sysdate-8) from dual) and (select TO_CHAR(sysdate-1) from dual)
__SQL2__
...
sub buildHashes
{	our $databaseHandler=shift;
	...
	while($statementHandler->fetch()) 	{	    my $statementHandler2 = $databaseHandler->prepare($stmt2);	    my $initAmount;
		...
		if($initAmount == -1)
		{	
			if($registerStatus eq "A" || $registerStatus eq "N")
		    {   
...
sub sendMailMessage
{
	...
	else
	{
		if(defined $opt{'p'})
		{
			$msg = MIME::Lite->new(
	    	    From    => '"IT Service Center"<itservicecenter@paradies-na.com>',
	    	    To      => 'pollingdistribution@paradies-na.com',
	       		Subject => "Polling Status For $businessDate",
	        	Type    => 'text/html',
	        	Data    => $body
	    	);
		...
		else
		{
			$msg = MIME::Lite->new(
	    	    ...
	    	    Cc      => 'L2-SC@paradies-na.com, nevin.harton@paradies-na.com',
	       		Subject => "Polling Status For $businessDate",
	        	Type    => 'text/html',
	        	Data    => $body
	    	);
