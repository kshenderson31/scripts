...
if($conn=getDatabaseConnection("svc", "root", 'tiger31'))
{
	#buildJSON($conn);
	our $text;
	foreach our $ix(1..100)
	{
		$text.=", " if $ix != 1;
		$text.=sprintf('["%s","%s","%s","%s","%s","%s","%s","%s","%s"]', '0985', '01', 100, 200, 300, 400, 500, 600, 700);	
	}
	$json=sprintf('{"sEcho":%d, "iTotalRecords":%d, "iTotalDisplayRecords":%d, "aoColumns": [ {"sTitle": "Store", "sWidth": "60px", "bSortable": "true" }, { "sTitle": "Terminal" , "sWidth": "60px", "bSortable": "true", "sClass":"center" }, { "sTitle": "Date" , "sWidth": "60px", "bSortable": "true" }, { "sTitle": "Bgn Txn", "sWidth": "60px", "bSortable": "true" }, { "sTitle": "End Txn", "sWidth": "160px", "bSortable": "true" }, { "sTitle": "Count", "sWidth": "160px", "bSortable": "true" } ], "aaData": [%s]}', 20, 100, 100, $text);
